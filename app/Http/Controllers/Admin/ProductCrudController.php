	...
	public function store(StoreRequest $request)
	{
        ...
        foreach ($request->file('images') as $img) {
            if(isset($img) && !empty($img)) {
                ...
                $image = new Image();
                $image->filename = $destinationPath.'/'.$img->getClientOriginalName();
                $image->position = '0';
                $image->save();
...
//        $request->merge(array('images' => [$image->id]));
	...
	public function update(UpdateRequest $request)
	{
        ...
        //         $destinationPath = 'uploads';
        //         $img->move($destinationPath,$img->getClientOriginalName());
        //         $image = Image::where('id', 75)
        //                 ->update(['filename' => $destinationPath.$img->getClientOriginalName()]);
        //         $imagesArray[] = 75;
        //     }
        // }
        // }
        ...
        foreach ($request->file('images') as $img) {
            if(isset($img) && !empty($img)) {
                ...
                $img->move($destinationPath,$img->getClientOriginalName());
                $image = new Image();
                $image->filename = ''.$img->getClientOriginalName();
                $image->position = '0';
                $image->save();
                $imagesArray[] = $image->id;
        ...
        // $this->crud->updateColumnDetails('img', ['attribute' => $image->filename]);
        // $this->crud->addField([
        //     'name'  => 'img', 
        //     'type'  => '', 
        ...
        // DB::table('products')
        //     ->where('id', $id)
        //     ->update(['img' => $image->filename]);
            
        // return $id = $this->crud->description;
        // $this->crud->setAttribute('img', 'a');
